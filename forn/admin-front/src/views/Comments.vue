<template>
  <el-card>
    <el-table :data="comments" v-loading="loading">
      <el-table-column prop="id" label="ID" width="80" />
      <el-table-column prop="productId" label="商品ID" width="100" />
      <el-table-column prop="userId" label="用户ID" width="100" />
      <el-table-column prop="content" label="留言内容" min-width="300" />
      <el-table-column prop="createTime" label="留言时间" width="180" />
      <el-table-column label="操作" width="100">
        <template #default="{ row }">
          <el-button type="danger" link @click="remove(row.id)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <el-pagination v-model:current-page="pageNum" :page-size="pageSize" :total="total" layout="prev, pager, next" @current-change="loadComments" style="justify-content: center; margin-top: 20px" />
  </el-card>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import { getComments, deleteComment } from '../api'
import { ElMessage } from 'element-plus'

const loading = ref(false)
const pageNum = ref(1)
const pageSize = ref(10)
const total = ref(0)
const comments = ref([])

const loadComments = async () => {
  loading.value = true
  try {
    const res = await getComments({ pageNum: pageNum.value, pageSize: pageSize.value })
    comments.value = res.records
    total.value = res.total
  } catch (e) {} finally {
    loading.value = false
  }
}

const remove = async (id) => {
  try {
    await deleteComment(id)
    ElMessage.success('删除成功')
    loadComments()
  } catch (e) {}
}

onMounted(() => { loadComments() })
</script>
